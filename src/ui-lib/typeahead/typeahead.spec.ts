...
class TestComponent {
  ...
  findNothing = (text$: Observable<string>) => text$.pipe(map(text => []));

  findNull = (text$: Observable<string>) => text$.pipe(map(text => null));
  ...
  formatter = (obj: {id: number, value: string}) => `${obj.id} ${obj.value}`;

  uppercaseFormatter = s => `${s}`.toUpperCase();

  uppercaseObjFormatter = (obj: {value: string}) => `${obj.value}`.toUpperCase();
